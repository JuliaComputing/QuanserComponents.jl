...
`Clock` object `clk`, and the resolved stabilizer gain/saturation values
`Ldef`/`umaxdef` used to populate the runtime-settable `StaticGains` struct.
...
function build_discrete_controller(; Ts = 0.005, L = nothing, umax = nothing, overrides...)
    ...
    # carry no defaults, so these two are resolved here (values may be expressions of
    # other parameters, so substitute to a fixed point).
    dv = ModelingToolkit.default_values(ModelingToolkit.expand_connections(sys))
    Ldef    = _resolve_value(dv[ModelingToolkit.unwrap(lqr_stab.L)], dv)
    umaxdef = _resolve_value(dv[ModelingToolkit.unwrap(lqr_stab.umax)], dv)

    return (; sys, swingup, clock, clk = ModelingToolkit.Clock(Ts), Ldef, umaxdef)
end
...
function _instantiate(gen; L = nothing, umax = nothing,
    ...
    Lv = collect(float.(something(L, gen.controller.Ldef)))
    umaxv = float(something(umax, gen.controller.umaxdef))
    function consume(m)
