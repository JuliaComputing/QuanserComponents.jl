# The MPC swing-up program (`FurutaMPCHardware`, `MPCController`): compiled once, then ticked
# against a simulated plant through the hardware I/O callbacks, as the swing-up program is in
# runtests.jl. The plant is the MPC's own prediction model -- the multibody `QubePendulum` ODE
# from `furuta_mpc_dynamics()` -- integrated with RK4, so this checks the program end to end
# (angle wrapping, velocity estimation, the swing-up/MPC switch and the acados solve) rather
# than the model.
...
@testset "MPC swing-up program" begin
    Ts = 0.005
    ...
    function simulate(x0; Tf)
        ...
        N = round(Int, Tf / Ts)
        elbow = zeros(N); shoulder = zeros(N); flags = zeros(N); stab = falses(N); us = zeros(N)
        for i in 1:N
            out = ctrl()
            @test isfinite(out.u) && abs(out.u) <= 10
            shoulder[i] = x[1]; elbow[i] = mod(x[2], 2pi); flags[i] = out.exitflag
            stab[i] = out.stabilizing > 0.5; us[i] = out.u
            for _ in 1:5
        ...
        (; shoulder, elbow, flags, stab, us)
    end

    # From hanging down: swings up, the MPC takes over, and the pendulum stays upright with
    # the arm centred. (The energy swing-up itself does not respect the end stops, only the
    # MPC does -- see test/mpc_rollouts.jl for the statistics.)
    r = simulate([0.0, 0.01, 0.0, 0.0]; Tf = 8.0)
    @test QC.hardware_counters() == (n_measure = 1600, n_write = 1600)
    @test any(r.stab)
    @test all(abs.(r.elbow[end-200:end] .- pi) .< 0.05)
    @test abs(r.shoulder[end]) < 0.1
    @test count(!=(0), r.flags[r.stab]) <= 2             # at most a stray failed solve; `warm_start = None` recovers

    # From a perturbed upright state the MPC catches it directly, keeping the arm inside the
    # end stops. (The velocity estimators start cold, so the first tick sees a spurious
    # velocity spike; the solver must shrug it off.)
    r = simulate([0.3, pi - 0.25, 0.0, 0.0]; Tf = 3.0)
    @test r.stab[1]
    @test all(abs.(r.elbow[end-100:end] .- pi) .< 0.02)
