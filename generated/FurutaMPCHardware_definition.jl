...
`compile_program` in src/program.jl and the program's `ProgramSpec` in src/mpc.jl.
...
`output_trajectories = true` makes the MPC record them. `run_ode!` in
src/program.jl does the run.
